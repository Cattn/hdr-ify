export const toP3 = (color: string | null | undefined): { color: string; r: string; g: string; b: string; a: string; } | undefined => {
...
export const rgbToP3 = (rgb: string): { color: string; r: string; g: string; b: string; a: string } => {
    const clamp255 = (v: number) => Math.min(255, Math.max(0, v));
    const clamp01 = (v: number) => Math.min(1, Math.max(0, v));
...
function parseRgbLike(input: string): { red: number; green: number; blue: number; alpha: number } | null {
    if (!input) return null;
    const s = input.trim().toLowerCase();
    const m = s.match(/^rgba?\((.*)\)$/i);
    if (!m) return null;
    const body = m[1].replace(/\s*\/\s*/g, ',');
    const parts = body.includes(',') ? body.split(',').map(p => p.trim()).filter(Boolean) : body.trim().split(/\s+/);
